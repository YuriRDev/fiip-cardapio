  ...
  const handleEnviarDelivery = async () => {
    ...
    await api.post('/orderDelivery', {
      ...
      if (barTelefone) {
        ...
        texto = (`*Fiip Delivery* %0A--------------------------------%0A%0A ${itemsComprados} *Taxa de entrega*: R$ ${frete.toFixed(2).replace('.', ',')} %0A *Total: R$ ${carrinhoPrice + frete}* %0A%0A--------------------------------%0A%0A*${nome}*%0A${telefone}%0A%0A${address}, ${addressNum} - ${compl} %0A${cep}%0A%0A*Pagamento*:  ${paymentMethod == 1 ? 'Cartão de crédito' : paymentMethod == 2 ? 'Cartão de débito' : paymentMethod == 0 ? 'Dinheiro' : ''} %0A ${paymentMethod == 0 ? troco ? `*Troco para: *R$ ${Number(troco.split('R$ ')[1]).toFixed(2).replace('.', ',')}` : '' : ''}  `)
